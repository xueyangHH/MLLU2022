...
class TestBoolQDataset(unittest.TestCase):
    ...
    def test_item(self):
        ...
        for i in range(self.boolq_dataset.__len__()):
          output = self.boolq_dataset.__getitem__(i)
          self.assertEqual(list(output.keys()), ['input_ids', 'attention_mask', 'labels'])
          self.assertEqual(len(output['input_ids']), self.max_seq_len)
          self.assertEqual(len(output['attention_mask']), self.max_seq_len)
          self.assertEqual(output['input_ids'].dtype, torch.long)
          self.assertEqual(output['attention_mask'].dtype, torch.long)
          self.assertIsInstance(output['labels'], int)
